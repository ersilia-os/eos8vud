...
class ROCSDataset_point_cloud(torch_geometric.data.Dataset):
  def __init__(
    ...
    mols,
    max_future_rocs,
    max_future_rocs_evaluated_dihedrals,
    ...
    atoms_pointer,
    bonds_pointer,
    dihedral_indices_array,
    dihedral_indices_pointer,
    indices_partial_before_array,
    indices_partial_before_pointer,
    indices_partial_after_array,
    indices_partial_after_pointer,
    query_indices_array,
    query_indices_pointer,
    N_points,
    N_rot,
    dihedral_var=0.0,
  ):
    ...
    self.max_future_rocs_evaluated_dihedrals = (
      max_future_rocs_evaluated_dihedrals  # for ALL data
    )
    self.max_future_rocs_index = (
      max_future_rocs_index  # specific to training/validation splits
    )
  ...
  def augment_perturb_structure(
    self, mol, partial_indices, dihedral_indices, dihedral_var=15.0
  ):
    ...
    for bond_ in rot_bonds_partial:
      if int(bond_[0]) == int(dihedral_indices[3]):
        ...
        p1 = rdkit.Chem.rdmolops.GetShortestPath(
          mol, int(bond_[0]), int(dihedral_indices[3])
        )
      if int(bond_[1]) == int(dihedral_indices[3]):
        p2 = ()
      else:
        p2 = rdkit.Chem.rdmolops.GetShortestPath(
          mol, int(bond_[1]), int(dihedral_indices[3])
        )
      ...
      if (
        (len(first_neighbors) > 0)
        & (len(second_neighbors) > 0)
        & (sorted(dihedral_indices[1:3]) != sorted(bond))
      ):
  ...
  def __getitem__(self, key):
    ...
    max_future_rocs_evaluated_dihedrals_array = deepcopy(
      self.max_future_rocs_evaluated_dihedrals[max_future_rocs_index]
    )

    dihedral_indices = deepcopy(
      self.dihedral_indices_array[
        self.dihedral_indices_pointer[key] : self.dihedral_indices_pointer[key + 1]
      ]
    )
    dihedral_indices = tuple([int(d) for d in dihedral_indices])

    indices_partial_before = deepcopy(
      self.indices_partial_before_array[
        self.indices_partial_before_pointer[key] : self.indices_partial_before_pointer[
          key + 1
        ]
      ]
    )
    indices_partial_after = deepcopy(
      self.indices_partial_after_array[
        self.indices_partial_after_pointer[key] : self.indices_partial_after_pointer[
          key + 1
        ]
      ]
    )
    query_indices = deepcopy(
      self.query_indices_array[
        self.query_indices_pointer[key] : self.query_indices_pointer[key + 1]
      ]
    )
    ...
    unknown_positions = (
      sorted(disjoint_graphs[1])
      if len(set(indices_partial_before).intersection(set(disjoint_graphs[0])))
      > len(set(indices_partial_before).intersection(set(disjoint_graphs[1])))
      else sorted(disjoint_graphs[0])
    )  # this should ONLY contain the indices of nodes whose positions are influenced by the dihedral (as well as the focal root node)

    N = deepcopy(self.N_rot)
    rotated_partial_positions_list = []

    selected_dihedrals = np.random.choice(
      range(1, max_future_rocs_evaluated_dihedrals_array.shape[0]),
      size=N - 1,
      replace=False,
    )
    ...
    mol_augmented = self.augment_perturb_structure(
      mol, indices_partial_before, dihedral_indices, dihedral_var=self.dihedral_var
    )
    ...
    for r, rot in enumerate(rots):
      ...
      rdkit.Chem.rdMolTransforms.SetDihedralDeg(
        mol_rotated.GetConformer(), *dihedral_indices, rot
      )

      rotated_pos = np.array(mol_rotated.GetConformer().GetPositions())
      rotated_partial_positions_list.append(
        rotated_pos[subgraph_node_index] - center_of_mass
      )
    ...
    add_to_edge_index = (
      np.concatenate(
        [np.ones(edge_index.shape, dtype=int) * i for i in range(N)], axis=1
      )
      * x.shape[0]
    )
    add_to_subgraph_node_index = (
      np.concatenate(
        [np.ones(subgraph_node_index.shape, dtype=int) * i for i in range(N)], axis=0
      )
      * x.shape[0]
    )
    ...
    cloud_temp_list = [
      get_point_cloud_numpy(pos, self.N_points, per_node=True) for i in range(N)
    ]
    cloud_list, cloud_batch_indices_list = (
      [t[0] for t in cloud_temp_list],
      [t[1] for t in cloud_temp_list],
    )
    ...
    add_to_edge_index_subgraph = (
      np.concatenate(
        [np.ones(edge_index_subgraph.shape, dtype=int) * i for i in range(N)], axis=1
      )
      * x_subgraph.shape[0]
    )
    add_to_query_index_subgraph = (
      np.concatenate(
        [np.ones(query_index_subgraph.shape, dtype=int) * i for i in range(N)], axis=0
      )
      * x_subgraph.shape[0]
    )
    ...
    edge_index_subgraph_repeat = (
      np.tile(edge_index_subgraph, (1, N)) + add_to_edge_index_subgraph
    )
    subgraph_node_index_repeat = (
      np.tile(subgraph_node_index, N) + add_to_subgraph_node_index
    )
    edge_attr_subgraph_repeat = np.tile(edge_attr_subgraph, (N, 1))
    query_index_subgraph_repeat = (
      np.tile(query_index_subgraph, N) + add_to_query_index_subgraph
    )
    atom_fragment_associations_subgraph_repeat = np.tile(
      atom_fragment_associations_subgraph, N
    )
    ...
    cloud_subgraph_temp_list = [
      get_point_cloud_numpy(p, self.N_points, per_node=True)
      for p in rotated_partial_positions_list
    ]
    ...
    cloud_batch_indices_subgraph = np.concatenate(
      cloud_batch_indices_subgraph_list, axis=0
    )
    ...
    data = ROCS_PairData(
      ...
      atom_fragment_associations=torch.from_numpy(
        atom_fragment_associations_repeat
      ).type(torch.long),
      ...
      query_index_subgraph=torch.from_numpy(query_index_subgraph_repeat).type(
        torch.long
      ),
      ...
      new_batch=torch.from_numpy(
        np.concatenate([np.ones(x.shape[0], dtype=int) * i for i in range(N)], axis=0)
      ).type(torch.long),
      new_batch_subgraph=torch.from_numpy(
        np.concatenate(
          [np.ones(x_subgraph.shape[0], dtype=int) * i for i in range(N)], axis=0
        )
      ).type(torch.long),
    )
