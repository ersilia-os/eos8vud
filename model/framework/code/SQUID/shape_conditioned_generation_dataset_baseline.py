...
reference_mols = [
  deepcopy(all_reference_mols[i][0]) for i in range(len(all_reference_mols))
]
...
artificial_mols = pd.read_pickle(
...
for i in tqdm(range(len(reference_mols))):
  ...
  screening_tanimoto_similarity = [
    rdkit.DataStructs.FingerprintSimilarity(*[
      rdkit.Chem.RDKFingerprint(x) for x in [mol, r_m]
    ])
    for r_m in random_mols
  ]
...
np.save(
  f"paper_results/dataset_baseline_shape_score_results.npy",
  np.array(screening_shape_score_results),
)
np.save(
  f"paper_results/dataset_baseline_tanimoto_results.npy",
  np.array(screening_tanimoto_results),
)
