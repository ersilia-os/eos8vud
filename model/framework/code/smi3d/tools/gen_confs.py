...
def genConfEnsemble(
  mol: Chem.Molecule, conf_gen: ConfGen.ConformerGenerator
) -> (int, int):
  ...
  if (
    status == ConfGen.ReturnCode.SUCCESS
    or status == ConfGen.ReturnCode.TOO_MUCH_SYMMETRY
  ):
  ...
  try:
    while reader.read(mol):
      ...
      try:
        ...
        if (
          status != ConfGen.ReturnCode.SUCCESS
          and status != ConfGen.ReturnCode.TOO_MUCH_SYMMETRY
        ):
          if args.quiet:
            print(
              "Error: conformer ensemble generation for molecule %s failed: %s"
              % (mol_id, status_to_str[status])
            )
          else:
            print(
              " -> Conformer ensemble generation failed: %s" % status_to_str[status]
            )

        elif not args.quiet:  # arrives here only if no severe error occurred
          if status == ConfGen.ReturnCode.TOO_MUCH_SYMMETRY:
            print(
              " -> Generated %s conformers (warning: too much top. symmetry - output ensemble may contain duplicates)"
              % str(num_confs)
            )
          else:
        ...
        if num_confs > 0:
          if not writer.write(mol):
            sys.exit(
              "Error: output of conformer ensemble for molecule %s failed" % mol_id
            )

      except Exception as e:
        sys.exit(
          "Error: conformer ensemble generation or output for molecule %s failed: %s"
          % (mol_id, str(e))
        )
...
def parseArgs() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
    description="Generates conformer ensembles for the given input molecules."
  )

  parser.add_argument(
    "-i", dest="in_file", required=True, metavar="<file>", help="Molecule input file"
  )
  parser.add_argument(
    "-o",
    dest="out_file",
