...
def oemol_from_rdmol(rdkitmol):
  ...
  for oeb, idx1, idx2, oestereo in stereo_bonds:
    oeb.SetStereo(
      [map_atoms[idx1], map_atoms[idx2]], oechem.OEBondStereo_CisTrans, oestereo
    )
...
def rocs_screening(query_list, reference):
  ...
  for query_mol in query_list:
    ...
    oeshape.OESortOverlayScores(
      scoreiter, overlay.Overlay(fitmol), oeshape.OEHighestTanimoto()
    )

    for (
      score
    ) in scoreiter:  # only 1 conformer, so only 1 score (iterates through 1 time)
    ...
    try:
      ...
      scores.append((
        score.GetTanimoto(),
        get_ROCS_mols(reference, outmol_rdkit).item(),
      ))
...
if __name__ == "__main__":
  ...
  def logger(
    text,
    file="optimization_results_MOSES2/optimization_shape_screening_scores_MOSES2_log.txt",
    save=True,
  ):
    if save:
  ...
  for r in range(len(reference_mols)):
    ...
    query_chunks = [
      query_mols[num_per_chunk * i : num_per_chunk * (i + 1)]
      for i in range(math.ceil(len(query_mols) / num_per_chunk))
    ]
    logger("processing chunks...")
    chunk_scores = []
    pool = Pool(18)
    for scores in tqdm(
      pool.imap(partial(rocs_screening, reference=reference), query_chunks),
      total=N_chunks,
    ):
    ...
    with open(
      "optimization_results_MOSES2/training_set_shape_screening_scores.pkl", "wb"
    ) as f:
