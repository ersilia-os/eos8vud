...
def retrieve_atom_ID(atom_features, atom_lookup):
  atom_ID = [
    i
    for i in range(atom_lookup.shape[0])
    if np.array_equal(atom_features, atom_lookup[i])
  ][0]
  return atom_ID
...
def fix_bond_angles_and_distance(mol, bond_lookup=None, unique_atoms=None):
  ...
  for p in list(rdkit.Chem.rdmolops.FindAllPathsOfLengthN(conf, 2, useBonds=True)):
    ...
    if (
      (conf.GetAtomWithIdx(atom_1).GetAtomicNum() == 1)
      | (conf.GetAtomWithIdx(atom_2).GetAtomicNum() == 1)
      | (conf.GetAtomWithIdx(atom_3).GetAtomicNum() == 1)
      | (conf.GetAtomWithIdx(atom_4).GetAtomicNum() == 1)
    ):
      continue
  ...
  for bond in conf.GetBonds():
    ...
    if (bond.GetBeginAtom().IsInRing() == False) | (
      bond.GetEndAtom().IsInRing() == False
    ):
      atom1_ID = retrieve_atom_ID(
        node_features[bond.GetBeginAtomIdx()], unique_atoms[1:]
      )
      ...
      true_bond_distance = rdkit.Chem.rdMolTransforms.GetBondLength(
        mol.GetConformer(), bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
      )

      rdkit.Chem.rdMolTransforms.SetBondLength(
        conf.GetConformer(), bond.GetBeginAtomIdx(), bond.GetEndAtomIdx(), bond_distance
      )
...
def fix_relative_dihedrals(conf):
  ...
  rot_bonds = (
    rot_bonds + reversed_rot_bonds
  )  # we want to consider each side of the rotatable bond separately
  ...
  for bond in rot_bonds:
    ...
    first_neighbors = tuple(
      set([a.GetIdx() for a in conf.GetAtomWithIdx(bond[0]).GetNeighbors()])
      - set([bond[1]])
    )
    second_neighbors = tuple(
      set([a.GetIdx() for a in conf.GetAtomWithIdx(bond[1]).GetNeighbors()])
      - set([bond[0]])
    )
    ...
    d4_anchor = second_neighbors[
      0
    ]  # this neighbor will be locked in place to serve as the reference dihedral
    ...
    if hybrid == "SP2":
      ...
      raise Exception(
        f"Do not have coupled relative angles for atom {d3} with hybridization {hybrid}"
      )

    for d4_query in second_neighbors[1:]:
      conf_backup = deepcopy(conf)

      bond_angle = rdkit.Chem.rdMolTransforms.GetAngleDeg(
        conf.GetConformer(), d1, d2, d3
      )
      if np.abs(bond_angle - 180.0) <= 5.0:
        # the alignment doesn't work as intended for linear chains like N#N-C-(R)(R)
        continue

      conf1 = deepcopy(conf)

      d_conf_1 = rdkit.Chem.rdMolTransforms.GetDihedralDeg(
        conf.GetConformer(), d1, d2, d3, d4_anchor
      )
      d_conf_2 = rdkit.Chem.rdMolTransforms.GetDihedralDeg(
        conf.GetConformer(), d1, d2, d3, d4_query
      )
      ...
      rdkit.Chem.rdMolTransforms.SetDihedralDeg(
        conf1.GetConformer(), d1, d2, d3, d4_query, attempt
      )
      rmse = rdkit.Chem.rdMolAlign.AlignMol(
        conf1, conf, atomMap=[(int(at), int(at)) for at in (d1, d2, d3)]
      )  # align to dihedral that isn't being specifically rotated (e.g., the focal rotatable bond)

      G = deepcopy(
        get_substructure_graph_for_matching(conf1, list(range(0, conf1.GetNumAtoms())))
      )
      ...
      alignment_indices = (
        sorted(disjoint_graphs[0])
        if d4_query in disjoint_graphs[0]
        else sorted(disjoint_graphs[1])
      )
...
def fix_local_geometries(m, bond_lookup, unique_atoms):
  m_a = deepcopy(m)
  m_a = fix_bond_angles_and_distance(
    m_a, bond_lookup=bond_lookup, unique_atoms=unique_atoms
  )
...
# replace this with whatever list of smiles you care about
test_smiles = pd.read_csv("data/MOSES2/test_MOSES.csv")
...
fragment_smiles_library = list([s for s in AtomFragment_database.smiles if len(s) > 0])
failed_filter = 0
test_mols = []
test_mols_artificial = []
for t in tqdm(test_smiles.SMILES):
  try:
    m_ = rdkit.Chem.MolFromSmiles(t)
    ...
    rdkit.Chem.rdmolops.AssignStereochemistryFrom3D(m)

    p = filter_mol(m, unique_atoms, fragment_smiles_library)

    if p:
      m_artificial = fix_local_geometries(m, bond_lookup, unique_atoms)

      test_mols.append(m)
      test_mols_artificial.append(m_artificial)
...
test_mol_df = pd.DataFrame()
test_mol_df["smiles"] = [rdkit.Chem.MolToSmiles(m) for m in test_mols]
test_mol_df["smiles_nostereo"] = [
  rdkit.Chem.MolToSmiles(m, isomericSmiles=False) for m in test_mols
]
test_mol_df["mol"] = test_mols
test_mol_df["artificial_mol"] = test_mols_artificial
test_mol_df["N_atoms"] = [m.GetNumAtoms() for m in test_mols]

test_mol_df.to_pickle("data/MOSES2/test_MOSES_filtered_artificial_mols.pkl")
