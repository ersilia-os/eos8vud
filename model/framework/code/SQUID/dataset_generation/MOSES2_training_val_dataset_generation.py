...
if not skip:
  ...
  pool = Pool()
  for i, m in tqdm(
    enumerate(pool.imap(conformer_generation, smiles)), total=len(smiles)
  ):
    if m is not None:
      mols.append(m)
  pool.close()
  ...
  rdkit_smiles_nostereo = [
    rdkit.Chem.MolToSmiles(m, isomericSmiles=False) for m in mols
  ]
...
def get_fragment_smiles(mol, ring_fragment):
  ...
  try:
    ...
    print(
      f"failed to extract fragment smiles from mol: {rdkit.Chem.MolToSmiles(mol)}, {ring_fragment}"
    )
    print(f"    {e}")
    return None
...
if not skip:
  ...
  for inc, mol in enumerate(database.drop_duplicates("ID").rdkit_mol_cistrans_stereo):
    if inc in [int((total / 20) * j) for j in range(1, 21)]:
      logger(f"    {(inc / total) * 100.0} % complete...")
  ...
  top_k_fragments = {
    k: v
    for k, v in sorted(fragments_smiles.items(), key=lambda item: item[1], reverse=True)
  }
  ...
  top_100_fragment_database.to_pickle(
    "data/MOSES2/MOSES2_top_100_fragment_database.pkl"
  )

else:
  top_100_fragment_database = pd.read_pickle(
    "data/MOSES2/MOSES2_top_100_fragment_database.pkl"
  )
...
if not skip:
  ...
  for inc, mol in enumerate(SMILES_nostereo_reduced_dataset.rdkit_mol_cistrans_stereo):
    ...
    for frag in ring_fragments:
      ...
      filtered_database_SMILES_nostereo.append(
        SMILES_nostereo_reduced_dataset.iloc[m].SMILES_nostereo
      )
    m += 1
  ...
  filtered_database = pd.read_pickle(
    "data/MOSES2/MOSES2_training_val_filtered_database.pkl"
  )
...
if not skip:
  ...
  train_smiles_df["N_atoms"] = [
    rdkit.Chem.MolFromSmiles(s).GetNumAtoms() for s in train_smiles
  ]
  train_smiles_df["N_acyclic_single_bonds"] = [
    len(get_acyclic_single_bonds(rdkit.Chem.MolFromSmiles(s))) for s in train_smiles
  ]

  val_smiles_df = pd.DataFrame()
  val_smiles_df["SMILES_nostereo"] = val_smiles
  val_smiles_df["N_atoms"] = [
    rdkit.Chem.MolFromSmiles(s).GetNumAtoms() for s in val_smiles
  ]
  val_smiles_df["N_acyclic_single_bonds"] = [
    len(get_acyclic_single_bonds(rdkit.Chem.MolFromSmiles(s))) for s in val_smiles
  ]
...
if not skip:
  ...
  for i, mol in enumerate(
    filtered_database.drop_duplicates("ID")["rdkit_mol_cistrans_stereo"]
  ):
    if i in [int((total / 20) * n) for n in range(1, 21)]:
      logger(f"    {(i / total) * 100.0} % complete...")
    ...
    for f, feat in enumerate(node_features):
      ...
      if list(feat) not in unique_atoms.tolist():
        unique_atoms = np.concatenate(
          [unique_atoms, np.expand_dims(feat, axis=0)], axis=0
        )  # np.unique(np.concatenate([unique_atoms, np.expand_dims(feat, axis = 0)], axis = 0), axis = 0)
        atom = atoms[f]
...
if not skip:
  ...
  AtomFragment_database["atom_features"] = [
    ar
    for ar in np.concatenate((
      unique_atoms,
      -1 * np.ones((len(top_100_fragments_mols), unique_atoms.shape[1])),
    ))
  ]
  AtomFragment_database["is_fragment"] = [0] * unique_atoms.shape[0] + [1] * len(
    top_100_fragments_mols
  )
  AtomFragment_database["smiles"] = [""] * unique_atoms.shape[
    0
  ] + top_100_fragments_smiles
  AtomFragment_database["equiv_atoms"] = [
    list(rdkit.Chem.CanonicalRankAtoms(m, breakTies=False)) if m != None else [0]
    for m in AtomFragment_database.mol
  ]
  ...
  for atom in unique_atoms_rdkit:
    ...
    for i in range(n_single):
      ...
      atom_rw.GetBondBetweenAtoms(0, i + 1).SetBondType(
  ...
  AtomFragment_database["atom_objects"] = atom_objects + [None] * len(
    top_100_fragments_mols
  )

  bond_counts = [
    translate_node_features(feat)[4:8] for feat in AtomFragment_database.atom_features
  ]
  N_single = [
    i[0]
    if AtomFragment_database.iloc[idx].is_fragment == 0
    else sum([
      a.GetTotalNumHs() for a in AtomFragment_database.iloc[idx].mol.GetAtoms()
    ])
    for idx, i in enumerate(bond_counts)
  ]
  N_double = [
    i[1] if AtomFragment_database.iloc[idx].is_fragment == 0 else 0
    for idx, i in enumerate(bond_counts)
  ]
  N_triple = [
    i[2] if AtomFragment_database.iloc[idx].is_fragment == 0 else 0
    for idx, i in enumerate(bond_counts)
  ]
  N_aromatic = [
    i[3] if AtomFragment_database.iloc[idx].is_fragment == 0 else 0
    for idx, i in enumerate(bond_counts)
  ]
  ...
  AtomFragment_database.to_pickle(
...
if not skip:
  ...
  for m_idx, m in enumerate(filtered_database.rdkit_mol_cistrans_stereo):
    if m_idx in [int((total / 20.0) * n) - 1 for n in range(1, 21)]:
      logger(f"    {((m_idx + 1) / total) * 100.0} % complete...")
    ...
    for i, atom in enumerate(atoms):
      ...
      if len(ring_fragment) > 0:
        ...
        atom_fragment_ID_index = np.where(
          AtomFragment_database_smiles == np.string_(frag_ID_smiles)
        )[0][0]
      else:
  ...
  atom_fragment_associations_array = np.concatenate(
    all_atom_fragment_associations, axis=0
  )

  logger("saving arrays...")
  np.save("data/MOSES2/MOSES2_training_val_edge_index_array.npy", edge_index_array)
  np.save(
    "data/MOSES2/MOSES2_training_val_edge_features_array.npy", edge_features_array
  )
  np.save(
    "data/MOSES2/MOSES2_training_val_node_features_array.npy", node_features_array
  )
  ...
  edge_features_array = np.load(
    "data/MOSES2/MOSES2_training_val_edge_features_array.npy"
  )
  node_features_array = np.load(
    "data/MOSES2/MOSES2_training_val_node_features_array.npy"
  )
...
if not skip:
  ...
  for inc, mol_db in enumerate(
    ...
    for bond in mol_copy.GetBonds():
      ...
      bond_distance = rdkit.Chem.rdMolTransforms.GetBondLength(
        mol_copy.GetConformer(), int(atom1.GetIdx()), int(atom2.GetIdx())
      )
      ...
      if (bond_lookup[[0, 1, 2]] == bond_properties).all(1).any() == False:
        ...
        avg = bond_lookup.iloc[bond_ID][3] + (
          (bond_distance - bond_lookup.iloc[bond_ID][3]) / float(N)
        )
...
def decompose_from_seed(
  ...
  list_positions_before, list_positions_after, list_dihedrals = (
    results[1],
    results[2],
    results[3],
  )
  ...
  for j in range(len(results[0][0])):
    ...
    next_atom_fragment_indices = (
      tuple(sorted(list_2D_next_atom_fragment_indices[j]))
      if list_2D_next_atom_fragment_indices[j] != -1
      else list_2D_next_atom_fragment_indices[j]
    )
    focal_root_node = list_2D_focal_root_node[j]
    seed_tuple = tuple(sorted(seed))

    focal_indices_ref_partial = [
      np.where(np.array(flatten(partial_graph_indices)) == f)[0][0]
      for f in np.array(flatten(focal_indices))
    ]
    if next_atom_index != -1:
      focal_attachment_index_ref_partial = np.where(
        np.array(flatten(partial_graph_indices)) == focal_attachment_index
      )[0][0]
      focal_attachment_point_label_prob = np.zeros(len(focal_indices_ref_partial))
      focal_attachment_point_label_prob[
        focal_indices_ref_partial.index(focal_attachment_index_ref_partial)
      ] = 1.0
    ...
    if next_atom_index != -1:
      atom_fragment_associations = deepcopy(
        atom_fragment_associations_array[
          atoms_pointer[original_index] : atoms_pointer[original_index + 1]
        ]
      )
      ...
      if len(next_atom_fragment_indices) > 1:
        next_atom_fragment_mol = deepcopy(
          AtomFragment_database_mols[next_atom_fragment_ID_index]
        )
        next_atom_fragment_attachment_index = get_attachment_index_of_fragment(
          mol, next_atom_fragment_indices, next_atom_fragment_mol, next_atom_index
        )
        multi_hot_next_atom_fragment_attachment_points = tuple(
          get_multi_hot_attachment_points(
            next_atom_fragment_mol, int(next_atom_fragment_attachment_index)
          )
        )
      ...
      next_atom_fragment_ID_index = (
        0  # 0 # the first entry in the fragment library is the STOP token
      )
    ...
    if next_atom_index != -1:  # NOT STOP TOKEN
      attachment_bond_type = str(
        mol.GetBondBetweenAtoms(
          int(focal_attachment_index), int(next_atom_index)
        ).GetBondType()
      )
      bond_type_class_index_label = ["SINGLE", "DOUBLE", "TRIPLE", "AROMATIC"].index(
  ...
  for j in range(len(list_positions_before)):
    ...
    seed_tuple = tuple(sorted(seed))
    query_indices = tuple(
      sorted(list(set(positions_after_sorted) - set(positions_before_sorted)))
    )
    query_indices_ref_partial = tuple([
      positions_after_sorted.index(idx) for idx in query_indices
    ])
  ...
  for inc, mol in enumerate(filtered_database.rdkit_mol_cistrans_stereo):
    try:
      ...
      for seed in select_seeds:
        graph_construction_database, future_rocs_partial_subgraph_database = (
          decompose_from_seed(
            graph_construction_database,
            future_rocs_partial_subgraph_database,
            mol=mol,
    ...
    if inc in [int((total / 20.0) * n) - 1 for n in range(1, 21)]:
      logger(f"    {((inc + 1) / total) * 100.0} % complete...")

      saving_int += 1

      graph_construction_database_pandas = pd.DataFrame()
      graph_construction_database_pandas[
        [
          "original_index",
          ...
          "focal_attachment_index_ref_partial",
          "focal_attachment_point_label_prob",
          "next_atom_fragment_attachment_index",
          "multi_hot_next_atom_fragment_attachment_points",
          "bond_type_class_index_label",
        ]
      ] = graph_construction_database
      graph_construction_database_pandas["N_atoms_partial"] = [
        len(indices)
        for indices in graph_construction_database_pandas.partial_graph_indices_sorted
      ]

      future_rocs_partial_subgraph_database_pandas = pd.DataFrame()
      future_rocs_partial_subgraph_database_pandas[
        [
          "original_index",
          "positions_before_sorted",
          "positions_after_sorted",
      ...
      future_rocs_partial_subgraph_database_pandas["N_atoms_partial"] = [
        len(pos)
        for pos in future_rocs_partial_subgraph_database_pandas.positions_after_sorted
      ]

      graph_construction_database_pandas.to_pickle(
        f"data/MOSES2/uncombined_databases/MOSES2_training_val_canonical_terminalSeeds_unmerged_graph_construction_database_{saving_int}_reduced.pkl"
      )
      future_rocs_partial_subgraph_database_pandas.to_pickle(
        f"data/MOSES2/uncombined_databases/MOSES2_training_val_canonical_terminalSeeds_unmerged_future_rocs_database_{saving_int}_reduced.pkl"
      )
