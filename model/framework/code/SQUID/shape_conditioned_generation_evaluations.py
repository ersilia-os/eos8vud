...
model_3D.load_state_dict(
  torch.load(model_3D_PATH, map_location=next(model_3D.parameters()).device),
  strict=True,
)
model_3D.eval()
...
rocs_model_3D.load_state_dict(
  torch.load(rocs_model_3D_PATH, map_location=next(rocs_model_3D.parameters()).device),
  strict=True,
)
rocs_model_3D.eval()
...
for m_idx, m_ in enumerate(val_mols):
  ...
  for seed in select_seeds:
    ...
    try:
      _, updated_mol, _, _, _, N_rocs_decisions, _, _, _, chirality_scored = (
        generate_3D_mol_from_sequence(
          sequence=[],
      ...
      pred_rocs = get_ROCS(
        torch.tensor(np.array(updated_mol.GetConformer().GetPositions())),
        torch.tensor(np.array(mol.GetConformer().GetPositions())),
      )

      tanimoto = rdkit.DataStructs.FingerprintSimilarity(*[
        rdkit.Chem.RDKFingerprint(x) for x in [mol, updated_mol]
      ])

      rocs_shape_score, aligned_rocs, aligned_mol = ROCS_shape_overlap(
        [updated_mol], mol
      )[0]
