...
class EGNN_VN_Encoder_point_cloud(nn.Module):
  def __init__(
    ...
    self.EGNN_layers = nn.ModuleList([
      EGNN_static(
        ...
        + int(self.mix_shape_to_nodes)
        * self.num_components
        * 2
        * 3,  # input number of node features
    ...
    self.conv1 = VNLinearLeakyReLU(
      2, self.conv_dims[0] // 3
    )  # VNLinearLeakyReLU(2, 64//3)
    ...
    self.point_invariant_mlp = nn.Sequential(
      nn.Linear(
        self.num_components * 2 * 3 + self.h_dim,
        self.point_invariant_mlp_hidden_dim * 2,
      ),
      nn.LeakyReLU(negative_slope=0.2),
      nn.Linear(
        self.point_invariant_mlp_hidden_dim * 2, self.point_invariant_mlp_hidden_dim
      ),
      nn.LeakyReLU(negative_slope=0.2),
      nn.Linear(
        self.point_invariant_mlp_hidden_dim, self.point_invariant_mlp_hidden_dim
      ),
    )
  ...
  def forward(
    ...
    points_reshaped = (points.reshape(batch_size, -1, 3)).permute(
      0, 2, 1
    )  # (B)x(3)x(N_points_per_cloud)
    ...
    if self.mix_shape_to_nodes:
      ...
      x_mean_expanded = x_atom_pooled.sum(dim=-1, keepdim=True).expand(
        x_atom_pooled.size()
      )
      x_gnn_invariant, _ = self.std_feature_mix_shape_to_nodes(
        torch.cat((x_atom_pooled, x_mean_expanded), dim=1)
      )  # shape (B, dim, 3, N_nodes)
      x_gnn_invariant_reshaped = x_gnn_invariant.reshape(
        batch_size, -1, num_nodes
      )  # rotation-invariant features per node, B x F x N_nodes
      x_gnn_to_cat_to_h = x_gnn_invariant_reshaped.permute(0, 2, 1).reshape(
        batch_size * num_nodes, -1
      )  # B x F x N_nodes --(permute)--> B x N_nodes x F --(reshape)--> B*N_nodes x F
    ...
    for EGNN_layer in self.EGNN_layers[1:]:
      h, _, edge_feat = EGNN_layer(
        h, edge_index, pos, edge_attr=edge_feat, node_attr=None
      )

    # try variationally encoding h embeddings here...
    if (
      (self.variational_GNN)
      & (use_variational_GNN)
      & (self.variational_GNN_mol == False)
    ):
    ...
    h_reshaped = (
      h.unsqueeze(0).reshape(batch_size, -1, h.shape[1]).permute(0, 2, 1)
    )  # (B*N)x(F) -> (B)x(N)x(F) -> (B)x(F)x(N)

    # mixing invariant GNN node embeddings into VN equivariant node embeddings
    if self.mix_node_inv_to_equi:
      h_projected = self.project_h_embeddings(
      ...
      x_atom_pooled = self.Equi_linear_leaky_mixing(
        torch.cat(
          (x_atom_pooled.permute(0, 3, 1, 2), x_atom_pooled_mixed), dim=2
        ).permute(0, 2, 3, 1)
      )  # (B)x(C)x(3)x(N_nodes)
    ...
    x_invariant, trans = self.std_feature(
      x_cat_x_mean
    )  # x_invariant now has shape (B, dim, 3, N_nodes)

    x_invariant = x_invariant.reshape(
      batch_size, -1, num_nodes
    )  # rotation-invariant features per node, B x F x N_nodes

    #!#
    if (
      (self.variational_GNN_mol)
      & (use_variational_GNN)
      & (self.variational_GNN == False)
    ):
      ...
      h_mol_eps = (
        torch.randn_like(h_mol_mean) * variational_GNN_factor
      )  # use the same variational_GNN_factor

      if (
        interpolate_to_GNN_prior > 1e-4
      ):  # e.g., > 0.0 # use the same interpolate_to_GNN_prior
    ...
    if self.ablate_HvarCat:
      ...
      x_invariant = torch.cat(
        (x_invariant, h_reshaped), dim=1
      )  # concatenate h embeddings with x_invariant: (B)x(F)x(N)

    x_invariant = self.point_invariant_mlp(x_invariant.permute(0, 2, 1)).permute(
      0, 2, 1
    )
