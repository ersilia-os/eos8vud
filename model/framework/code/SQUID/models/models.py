...
class ROCS_Model_Point_Cloud(nn.Module):
  def __init__(
    ...
    if not self.subtract_latent_space:
      graph_subgraph_focal_features_concat_dim = (
        num_components * 3 + EGNN_layer_dim * 3
      )  # *3 is from flattening the dimensions from the (now invariant) equivariant latent space. *3 is from concatenating the invariant latent spaces (encoded, partial, focal)
    else:
      graph_subgraph_focal_features_concat_dim = (
        num_components * 3 + EGNN_layer_dim * 4
      )  # *4 is from concatenating the invariant latent spaces (encoded, partial, focal, encoded - partial)
...
class Model_Point_Cloud_Switched(nn.Module):
  ...
  def pairwise_predict(
    self, h_reshaped, pairwise_indices_1_select, pairwise_indices_2_select
  ):
    h_aggregated = h_reshaped.mean(
      -1
    )  # (B)x(F); mean aggregation so we can reasonably compare molecules of different sizes.
    h_pairwise_cat_ab = torch.cat(
      (
        h_aggregated[pairwise_indices_1_select],
        h_aggregated[pairwise_indices_2_select],
      ),
      dim=1,
    )
    h_pairwise_cat_ba = torch.cat(
      (
        h_aggregated[pairwise_indices_2_select],
        h_aggregated[pairwise_indices_1_select],
      ),
    ...
    h_pairwise_mixed = self.PairwiseMixing(h_pairwise_cat_ab) + self.PairwiseMixing(
      h_pairwise_cat_ba
    )  # symmetry preserving
  ...
  def forward(
    ...
    all_stop=False,
    pairwise_indices_1_select=None,
    pairwise_indices_2_select=None,
    device=torch.device("cpu"),
  ):
    ...
    if self.predict_pairwise_properties:
      pairwise_properties_out = self.pairwise_predict(
        h_reshaped, pairwise_indices_1_select, pairwise_indices_2_select
      )
    else:
