...
RUN pip install torchvision==0.12.0 --extra-index-url https://download.pytorch.org/whl/cpu
RUN pip install torch-geometric==2.5.3 
RUN pip install --no-build-isolation torch-scatter==2.1.2 -f https://data.pyg.org/whl/torch-1.11.0+cpu.html
