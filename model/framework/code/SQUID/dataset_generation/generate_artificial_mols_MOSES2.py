...
if __name__ == "__main__":
  ...
  for i in range(num):
    ...
    mols = deepcopy(
      all_mols[
        i * int((len(all_mols) / float(num)) + 1.0) : (i + 1)
        * int((len(all_mols) / float(num)) + 1.0)
      ]
    )
    ...
    pool = Pool()
    for i, tup in tqdm(
      enumerate(pool.imap(fix_bonding_geometries, mols)), total=len(mols)
    ):
      m_a, fail = tup
