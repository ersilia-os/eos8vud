...
def tanimoto_similarity(mol1, mol2):
  sim = rdkit.DataStructs.FingerprintSimilarity(*[
    rdkit.Chem.RDKFingerprint(x) for x in [mol1, mol2]
  ])
  return sim
...
S_thresh = (
  0.845  # target shape similarity constraint (>= 0.85; we round to nearest 0.01)
)
...
all_select_seeds = get_starting_seeds(
...
for p in tqdm(population):
  ...
  if len(gen_mol_list) == 0:
    continue
  if gen_mol_list[0] is None:
    continue
...
for iteration in range(1, iterations + 1):
  ...
  optimization_df.to_pickle(
    PATH + f"/optimization_results_{test_indices[ref_mol_idx]}_incomplete.pkl"
  )
  logger("saved partial results...")

  best_score_above_thresh_df = optimization_df[
    optimization_df.shape_scores_history >= S_thresh
  ].score_history
  best_score_above_thresh = (
    max(best_score_above_thresh_df) if len(best_score_above_thresh_df) > 0 else None
  )
  logger(f"top score: {best_score_above_thresh}")

  sorted_indices = sorted(
    range(len(score_history)), key=lambda k: score_history[k], reverse=True
  )
  ...
  for m_idx in sorted_indices:
    ...
    if len(top_indices) == N_top_scores:
      break
  ...
  random_mates = [
    tuple(randomized_top_indices[i : i + 2])
    for i in range(0, N_mates * 2, 2)
    if (i + 2) < len(randomized_top_indices)
  ]
  ...
  for H in top_H:
    for n in range(N_mutations_per):
      for interp in interpolation_values:
        H_mutated = mutate(
          H, interpolate_to_prior=interp, sample_std=sample_std_space[n]
        )
...
optimization_df.to_pickle(
  PATH + f"/optimization_results_{test_indices[ref_mol_idx]}_complete.pkl"
)
