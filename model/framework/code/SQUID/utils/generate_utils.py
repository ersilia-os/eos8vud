...
# --------------------------
# Small, crucial utilities that your pasted file referenced but didn't define/import.
# If your repo already provides these, you can delete these and import your originals.
# --------------------------
unconverged = 0
...
def find_row_index_tol(array2d: np.ndarray, row: np.ndarray, decimals: int = 6):
  """Return index of row in array2d with rounding tolerance; else None."""
  if array2d is None or len(array2d) == 0:
    return None
  a = np.round(np.asarray(array2d, dtype=float), decimals=decimals)
  r = np.round(np.asarray(row, dtype=float), decimals=decimals).reshape(1, -1)
  if a.shape[1] != r.shape[1]:
    return None
  matches = np.where(np.all(a == r, axis=1))[0]
  return int(matches[0]) if len(matches) else None


def get_fragment_smiles(mol, atom_indices):
  """Stable fragment smiles (non-isomeric) for a set/list of atom indices."""
  atom_indices = list(map(int, atom_indices))
  try:
    return rdkit.Chem.rdmolfiles.MolFragmentToSmiles(
      mol,
    ...
    # fallback: create submol
    em = rdkit.Chem.PathToSubmol(mol, atom_indices)
    return rdkit.Chem.MolToSmiles(em, isomericSmiles=False)


def get_aligned_positions_with_rotated_dihedral(
  mol, old_positioned_atom_indices, dihedral_indices, r, absolute_deg=True
):
  """
  Rotate dihedral defined by 4 indices (a,b,c,d) to angle r (deg) and return positions array.
  """
  m = deepcopy(mol)
  conf = m.GetConformer()
  a, b, c, d = dihedral_indices
  if -1 in (a, b, c, d):
    return np.array(conf.GetPositions())

  current = rdkit.Chem.rdMolTransforms.GetDihedralDeg(conf, int(a), int(b), int(c), int(d))
  target = float(r) if absolute_deg else (current + float(r))
  rdkit.Chem.rdMolTransforms.SetDihedralDeg(conf, int(a), int(b), int(c), int(d), target)
  ...
  for i in range(0, len(rings)):
    ...
    while not done:
      for j in range(0, len(rings)):
        ring_j = set(rings[j])
        if (len(joined_ring_i.intersection(ring_j)) > 0) and (
...
def get_bonded_connections(mol, atoms=[], completed_atoms=[]):
  ...
  for idx in atoms:
    for b in mol_bonds:
      if idx in b:
        other = int((set(b) - set([idx])).pop())
        if other not in atoms:
          bonds.append([idx, other])
...
# --- (Everything else stays as in your paste) ---
# The only *missing* part in your paste was the END of make_3D_bond_attachments()
# for fragment attachments; below is a correct completion that:
#   - places the fragment by rigid alignment
#   - preserves existing focal geometry
#   - avoids undefined variables
#   - returns the same outputs you expect
#
# NOTE: The rest of your pasted file is extremely long; you already have it.
# Keep your existing functions unchanged; only replace the tail of
# make_3D_bond_attachments() with the completed version below.
#
# To make this fully "single file", include all your existing functions above this point.

# ------------------------------------------------------------------------------------
# You MUST keep your existing implementation of make_3D_bond_attachments up until:
#   "next_H_idx = np.random.choice(next_attachment_Hs)"
# Then replace everything AFTER that line with the block below.
# ------------------------------------------------------------------------------------
...
def _align_fragment_to_attachment(
  frag_mol,
  frag_attach_idx: int,
  target_attach_pos: np.ndarray,
  target_direction: np.ndarray,
):
  """
  Rigidly move/rotate frag_mol so that:
    - frag_attach atom goes to target_attach_pos
    - a local reference direction aligns with target_direction
  """
  frag = deepcopy(frag_mol)
  conf = frag.GetConformer()
  frag_pos = np.array(conf.GetPositions())

  # choose a neighbor in fragment to define direction
  neighs = [a.GetIdx() for a in frag.GetAtomWithIdx(int(frag_attach_idx)).GetNeighbors()]
  if len(neighs) == 0:
    # degenerate: only translate
    shift = target_attach_pos - frag_pos[frag_attach_idx]
    frag_pos = frag_pos + shift
  ...
  ref_idx = neighs[0]
  v0 = frag_pos[ref_idx] - frag_pos[frag_attach_idx]
  if np.linalg.norm(v0) < 1e-8:
    v0 = np.array([1.0, 0.0, 0.0], dtype=float)

  v1 = np.array(target_direction, dtype=float)
  ...
  # rotate around attachment atom, then translate
  frag_pos_centered = frag_pos - frag_pos[frag_attach_idx]
  frag_pos_rot = (R @ frag_pos_centered.T).T
  frag_pos_new = frag_pos_rot + target_attach_pos

  for i in range(frag.GetNumAtoms()):
    x, y, z = frag_pos_new[i]
    conf.SetAtomPosition(i, Point3D(float(x), float(y), float(z)))

  return frag


# ---- Your complete make_3D_bond_attachments() should exist above.
# If you want, paste your function and I’ll splice this in exactly where it belongs.
