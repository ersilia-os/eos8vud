...
class MaxFutureRocsDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, key):
    ...
    G = deepcopy(
      get_substructure_graph_for_matching(
        original_mol, list(range(0, original_mol.GetNumAtoms()))
      )
    )
    ...
    unknown_positions = (
      ...
      if len(set(positions_before).intersection(set(disjoint_graphs[0])))
      > len(set(positions_before).intersection(set(disjoint_graphs[1])))
    ...
    query_and_searched_positions = [
      u for u in unknown_positions if u not in dihedral[1:3]
    ]
    ...
    future_rot_bonds = [
      ...
      if (
        (
          (b[0] in query_and_searched_positions)
          | (b[1] in query_and_searched_positions)
        )
        & (original_mol.GetAtomWithIdx(b[0]).GetDegree() > 1)
        & (original_mol.GetAtomWithIdx(b[1]).GetDegree() > 1)
      )
    ]
    ...
    for bond_ in future_rot_bonds:
      if int(bond_[0]) == int(dihedral[2]):
        ...
        p1 = rdkit.Chem.rdmolops.GetShortestPath(
          original_mol, int(bond_[0]), int(dihedral[2])
        )
      if int(bond_[1]) == int(dihedral[2]):
        p2 = ()
      else:
        p2 = rdkit.Chem.rdmolops.GetShortestPath(
          original_mol, int(bond_[1]), int(dihedral[2])
        )
      ...
      first_neighbors = tuple(
        set([a.GetIdx() for a in original_mol.GetAtomWithIdx(bond[0]).GetNeighbors()])
        - set([bond[1]])
      )
      second_neighbors = tuple(
        set([a.GetIdx() for a in original_mol.GetAtomWithIdx(bond[1]).GetNeighbors()])
        - set([bond[0]])
      )
    ...
    N_confs = max(
      200 * len(future_dihedrals) ** 1 + 1, 1800
    )  # min(10**len(future_dihedrals), 1000) #32 * 10 * 300
    ...
    alphas_batched_1 = (
      np.ones((N_confs, len(query_and_searched_positions))) * 2.0
    )  # 0.81
    alphas_batched_2 = (
      np.ones((N_confs, len(query_and_searched_positions))) * 2.0
    )  # 0.81
    ...
    for a, angle in enumerate(np.arange(0, 360, 10)):
      ...
      set_dihedral_angle = float(
        angle + rdkit.Chem.rdMolTransforms.GetDihedralDeg(mol.GetConformer(), *dihedral)
      )
      rdkit.Chem.rdMolTransforms.SetDihedralDeg(
        mol.GetConformer(), *dihedral, set_dihedral_angle
      )
      ...
      for i in range(N_confs):
        ...
        for d in future_dihedrals:
          rdkit.Chem.rdMolTransforms.SetDihedralDeg(
            conf.GetConformer(), *d, float(random_angles[it])
          )
          it += 1

        centers_batched_1[i] = conf.GetConformer().GetPositions()[
          query_and_searched_positions
        ]
        centers_batched_2[i] = original_mol.GetConformer().GetPositions()[
          query_and_searched_positions
        ]

      all_rocs_batched = shape_tanimoto_batched(
        torch.as_tensor(centers_batched_1),
        torch.as_tensor(centers_batched_2),
        torch.as_tensor(alphas_batched_1),
        torch.as_tensor(alphas_batched_2),
        torch.as_tensor(prefactors_batched_1),
        torch.as_tensor(prefactors_batched_2),
      ).numpy()
    ...
    return (
      torch.as_tensor(all_max_rocs),
      torch.as_tensor(dihedral_angles),
      torch.tensor([self.database_index[key]]).type(torch.long),
    )
...
artificial_mols_df = pd.read_pickle(
...
unmerged_future_rocs_db = (
  pd
  .read_pickle(
    "data/MOSES2/MOSES2_training_val_canonical_terminalSeeds_unmerged_future_rocs_database_all_reduced.pkl"
  )
  .drop_duplicates(["original_index", "dihedral_indices", "positions_before_sorted"])
  .reset_index(drop=True)
)
database = filtered_database[["original_index", "mol_artificial"]].merge(
  unmerged_future_rocs_db, on="original_index"
)
...
loader = torch.utils.data.DataLoader(
  dataset, shuffle=False, batch_size=4, num_workers=24
)
...
for b, batch in tqdm(enumerate(loader), total=len(loader)):
  ...
  if (b + 1) % 10000 == 0:
    ...
    np.save(
      f"data/MOSES2/max_future_rocs_data_artificial_alpha_2_0/computed_rocs_{split_idx}.npy",
      computed_rocs[0:pointer],
    )
    np.save(
      f"data/MOSES2/max_future_rocs_data_artificial_alpha_2_0/evaluated_dihedrals_{split_idx}.npy",
      evaluated_dihedrals[0:pointer],
    )
    np.save(
      f"data/MOSES2/max_future_rocs_data_artificial_alpha_2_0/evaluated_indices_{split_idx}.npy",
      evaluated_indices[0:pointer],
    )
