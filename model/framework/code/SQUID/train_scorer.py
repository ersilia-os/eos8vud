...
beta = (
  ...
  if ((variational == True) | (variational_GNN == True))
  else None
)
...
MOSES2_training_val_filtered_database_artificial_mols = pd.read_pickle(
  "data/MOSES2/MOSES2_training_val_filtered_database_artificial_mols.pkl"
)
filtered_database["rdkit_mol_cistrans_stereo"] = (
  MOSES2_training_val_filtered_database_artificial_mols.artificial_mols
)
...
unmerged_future_rocs_db["max_future_rocs_index"] = range(
  0, len(unmerged_future_rocs_db)
)
...
].merge(unmerged_future_rocs_db, on="original_index")
...
s = 0
for i, p in tqdm(
  enumerate(train_database.positions_before_sorted), total=len(train_database)
):
...
s = 0
for i, p in tqdm(
  enumerate(train_database.positions_after_sorted), total=len(train_database)
):
...
val_indices_partial_before_array = np.array(
  flatten(val_database.positions_before_sorted)
)
val_indices_partial_before_pointer = np.zeros(len(val_database) + 1, dtype=int)
s = 0
for i, p in tqdm(
  enumerate(val_database.positions_before_sorted), total=len(val_database)
):
...
s = 0
for i, p in tqdm(
  enumerate(val_database.positions_after_sorted), total=len(val_database)
):
...
def loop(
  ...
  acc = torch.mean(
    (
      torch.argmax(out.squeeze().reshape(-1, N_rot), dim=1)
      == torch.argmax(rocs.squeeze().reshape(-1, N_rot), dim=1)
    ).type(torch.float)
  )
...
for epoch in range(1, 1 + N_epochs):
  ...
  for b, batch in enumerate(train_loader):
    batch_size, loss, mae, acc, KL_loss = loop(
      model,
      optimizer,
      batch,
      training=training,
    ...
    if iteration % interval == 0:
      train_loss.append(
        float(np.nansum(np.array(losses) * np.array(batch_sizes)))
      ...
      train_mae.append(
        float(np.nansum(np.array(MAEs) * np.array(batch_sizes)))
        / sum(np.array(batch_sizes))
      )
      train_acc.append(
        float(np.nansum(np.array(accs) * np.array(batch_sizes)))
        / sum(np.array(batch_sizes))
      )
      train_epoch_number.append(epoch)

      logger(
        f"iteration: {iteration}, epoch: {epoch}, batch: {b}, loss: {train_loss[-1]}, MAE: {train_mae[-1]}, acc: {train_acc[-1]}, KL_loss: {train_KL_loss[-1]}"
      )
      losses = []
      KL_losses = []
      MAEs = []
    ...
    if (save) & ((iteration % save_interval) == 0):
      ...
      torch.save(
        model.state_dict(),
        PATH + f"saved_models/rocs_model_{int(iteration / save_interval)}.pt",
      )
  ...
  for b, batch in enumerate(
    val_loader
  ):  # with chunks = 25, we only evaluate on a random 4% of the entire validation set
    batch_size, loss, mae, acc, KL_loss = loop(
      model,
      None,
      batch,
  ...
  val_loss.append(
    float(np.nansum(np.array(val_losses) * np.array(val_batch_sizes)))
  ...
  val_mae.append(
    float(np.nansum(np.array(val_MAEs) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )
  val_acc.append(
    float(np.nansum(np.array(val_accs) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )
  val_epoch_number.append(epoch)

  val_logger(
    f"iteration: {iteration - 1}, loss: {val_loss[-1]}, MAE: {val_mae[-1]}, acc: {val_acc[-1]}, KL_loss: {val_KL_loss[-1]}"
  )
