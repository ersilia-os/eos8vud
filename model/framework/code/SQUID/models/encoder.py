...
class Encoder_point_cloud(nn.Module):
  ...
  def mix_codes(
    ...
    if self.ablateEqui:
      Z_equi = torch.zeros_like(
        Z_equi
      )  # only ablating the equivariant information from the encoded molecule.
    ...
    Z_T_invariant = torch.einsum(
      "bij,bjk->bik", Z_equivariant, T_equivariant.permute(0, 2, 1)
    )
  ...
  def forward(
    ...
    if self.variational:
      if (self.variational_mode == "both") | (self.variational_mode == "equi"):
        ...
        Z_equi_mean, Z_equi_logvar = Z_equi.chunk(
          2, dim=1
        )  # equivariant, shape [B,C,3]
        Z_equi_logvar, _ = self.VariationalEncoder_equi_T(
          Z_equi_logvar
