...
def main() -> None:
  ...
  with open(args.in_file, "r") as f:
    ...
    if smiles_list[0].lower().startswith("smiles") or smiles_list[0].lower().endswith(
      "smiles"
    ):
  ...
  for i, smi in enumerate(smiles_list):
    ...
    mol.SetProp(
      "_InputInChIKey",
      str(inchi.InchiToInchiKey(inchi.MolToInchi(Chem.MolFromSmiles(smi)))),
    )
  ...
  if args.num_confs == 1:
    cmd = "{0} {1}/tools/gen_3d_structs.py -i {2} -o {3} -t {4}".format(
      python_path, root, input_file, output_file, args.max_time
    )
  else:
    cmd = "{0} {1}/tools/gen_confs.py -i {2} -o {3} -t {4} -n {5}".format(
      python_path, root, input_file, output_file, args.max_time, args.num_confs
    )
...
def parseArgs() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
    description="Generates conformers for a given list of SMILES strings."
  )
