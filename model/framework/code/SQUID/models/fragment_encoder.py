...
class FragmentLibraryEncoder(nn.Module):
  def __init__(
    ...
    self.EGNN_layers = nn.ModuleList([
      EGNN_static(
        input_nf=input_nf,  # input number of node features
    ...
    for layer in range(1, N_layers):
      self.EGNN_layers.append(
        EGNN_static(
          ...
          if ((layer < (N_layers - 1)) | (self.append_noise == False))
          else output_dim // 2,
          hidden_nf=output_dim,
    ...
    if self.append_noise:
      self.noise_embedding = torch.nn.Embedding(
        N_members, output_dim - (output_dim // 2)
      )

  def forward(
    self, x, edge_index, pos, edge_attr, batch_index, device=torch.device("cpu")
  ):
