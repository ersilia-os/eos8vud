...
def get_point_cloud_numpy(
  centers, N, per_node=True, var=1.0 / (12.0 * 1.7)
):  # var = 1./(4.*1.7) |or| var = 1./(12.*1.7)
  if per_node:
    N_points_per_atom = N
  else:
    N_total = N
  ...
  cloud_batch_index_all = np.concatenate(
    [np.ones(N_points_per_atom, dtype=int) * i for i in range(centers.shape[0])], axis=0
  )
...
class VNNBatchSampler(torch.utils.data.sampler.Sampler):
  ...
  def __iter__(
    self,
  ):  # returns list of lists of indices, with each inner list containing a batch of indices
  ...
  def __len__(self):  # number of batches
    return (
      sum([math.ceil(n_g / self.max_batch_size) for n_g in self.group_lengths])
      // self.chunks
    )
...
class Collater:
  def __init__(
    self, follow_batch, exclude_keys, N_fragment_library_nodes, fragment_batch_batch
  ):
  ...
  def process_batch(self, batch):
    ...
    focal_attachment_label_prob_masked = batch.focal_attachment_point_label_prob[
      batch.stop_focal_mask
    ]
    all_stop = True not in stop_mask

    masked_focal_batch_index = batch.x_subgraph_batch[batch.focal_index_subgraph][
      batch.stop_focal_mask
    ]
    focal_reindexing_map = {
      int(j): int(i)
      for i, j in zip(
        torch.arange(torch.unique(masked_focal_batch_index).shape[0]),
        torch.unique(masked_focal_batch_index),
      )
    }
    masked_focal_batch_index_reindexed = torch.tensor(
      [focal_reindexing_map[int(i)] for i in masked_focal_batch_index], dtype=torch.long
    )

    if not all_stop:
      next_atom_attachment_indices = torch.cat([
        torch.arange(self.N_fragment_library_nodes)[
          self.fragment_batch_batch == batch.next_atom_type_library_idx[i]
        ]
        for i in range(batch.next_atom_type_library_idx.shape[0])
        if stop_mask[i]
      ])
      masked_next_atom_attachment_batch_index = torch.tensor(
        np.concatenate([
          [i] * len(batch.multi_hot_next_atom_fragment_attachment_points[i])
          for i in range(len(batch.multi_hot_next_atom_fragment_attachment_points))
          if stop_mask[i]
        ]),
        dtype=torch.long,
      )
      next_atom_attachment_reindexing_map = {
        int(j): int(i)
        for i, j in zip(
          torch.arange(torch.unique(masked_next_atom_attachment_batch_index).shape[0]),
          torch.unique(masked_next_atom_attachment_batch_index),
        )
      }
      masked_next_atom_attachment_batch_index_reindexed = torch.tensor(
        [
          next_atom_attachment_reindexing_map[int(i)]
          for i in masked_next_atom_attachment_batch_index
        ],
        dtype=torch.long,
      )
      ...
      masked_multihot_next_attachments_label_prob = [
        ...
        for item in [
          j
          for i, j in enumerate(batch.next_atom_fragment_attachment_point_label_prob)
          if (stop_mask[i] == 1)
        ]
        for el in item
      ]
      masked_multihot_next_attachments = torch.tensor(
        [
          el
          for item in [
            j
            for i, j in enumerate(batch.multi_hot_next_atom_fragment_attachment_points)
            if (stop_mask[i] == 1)
          ]
          for el in item
        ],
      ...
      next_atomFragment_attachment_loss_mask = torch.tensor(
        [
          int(sum(masked_next_atom_attachment_batch_index_reindexed == i) > 1)
          for i in range(
            torch.unique(masked_next_atom_attachment_batch_index_reindexed).shape[0]
          )
        ],
        dtype=torch.bool,
      )
      next_atomFragment_attachment_loss_mask_size = int(
        sum(next_atomFragment_attachment_loss_mask)
      )
      ...
      if next_atomFragment_attachment_loss_mask_size > 0:
        mask_multi = (
          torch.sum(
            torch.cat(
              [
                (masked_next_atom_attachment_batch_index_reindexed == i).unsqueeze(0)
                for i in torch.unique(
                  masked_next_atom_attachment_batch_index_reindexed
                )[next_atomFragment_attachment_loss_mask]
              ],
              dim=0,
            ),
            dim=0,
        ...
        select_multi_losses = torch.arange(
          0, masked_next_atom_attachment_batch_index_reindexed.shape[0]
        )[mask_multi][
          torch.tensor(
            np.concatenate([
              i > 0.0
              for i in batch.next_atom_fragment_attachment_point_label_prob
              if len(i) > 1
            ])
          ).reshape(-1)
        ]
        temp_mask = torch.tensor(
          np.concatenate([
            (i > 0.0) * n
            for n, i in enumerate(batch.next_atom_fragment_attachment_point_label_prob)
            if len(i) > 1
          ])
        )
        temp_mask_bool = torch.BoolTensor(
          np.concatenate([
            i > 0.0
            for i in batch.next_atom_fragment_attachment_point_label_prob
            if len(i) > 1
        ...
        reindexing_temp = {
          int(j): int(i)
          for i, j in zip(
            torch.arange(torch.unique(temp_mask[temp_mask_bool]).shape[0]),
            torch.unique(temp_mask[temp_mask_bool]),
          )
        }
        mask_select_multi = torch.tensor(
          [reindexing_temp[int(i)] for i in temp_mask[temp_mask_bool]], dtype=torch.long
        )
      else:
      ...
      focal_attachment_loss_mask = torch.tensor(
        [
          int(sum(masked_focal_batch_index_reindexed == i) > 1)
          for i in range(torch.unique(masked_focal_batch_index_reindexed).shape[0])
        ],
        dtype=torch.bool,
      )
    ...
    values = np.random.uniform(
      low=0.0, high=1.0, size=500
    )  # approximate uniform sampling over tanimoto similarity values
    differences = values.reshape(1, -1) - sim.reshape(-1, 1)
    indices_ = np.abs(differences).argmin(axis=0)
    pairwise_indices_1_select = torch.from_numpy(indices_1[np.unique(indices_)]).type(
      torch.long
    )
    pairwise_indices_2_select = torch.from_numpy(indices_2[np.unique(indices_)]).type(
      torch.long
    )
    pairwise_targets = torch.from_numpy(
      sim[np.unique(indices_)]
    ...
    batch_dict["next_atomFragment_attachment_loss_mask_size"] = (
    ...
    batch_dict["focal_attachment_label_prob_masked"] = (
      focal_attachment_label_prob_masked
    )
    batch_dict["masked_focal_batch_index_reindexed"] = (
      masked_focal_batch_index_reindexed
    )
    batch_dict["focal_attachment_index_rel_partial"] = (
      focal_attachment_index_rel_partial
    )
    batch_dict["next_atom_attachment_indices"] = next_atom_attachment_indices
    batch_dict["masked_next_atom_attachment_batch_index_reindexed"] = (
      masked_next_atom_attachment_batch_index_reindexed
    )
    batch_dict["masked_multihot_next_attachments"] = masked_multihot_next_attachments
    batch_dict["masked_multihot_next_attachments_label_prob"] = (
      masked_multihot_next_attachments_label_prob
    )
  ...
  def __init__(
    ...
    **kwargs,
    ...
    super().__init__(
      ...
      shuffle,
      collate_fn=Collater(
        follow_batch, exclude_keys, N_fragment_library_nodes, fragment_batch_batch
      ),
      **kwargs,
    )
...
class PairData(torch_geometric.data.Data):
  def __init__(
    ...
    self.next_atom_fragment_attachment_point_label_prob = (
      next_atom_fragment_attachment_point_label_prob
    )
    self.multi_hot_next_atom_fragment_attachment_points = (
  ...
  def __inc__(self, key, value, *args, **kwargs):
    ...
    if key in [
      "edge_index_subgraph",
      "cloud_index_subgraph",
      "focal_index_subgraph",
      "focal_attachment_index_subgraph",
    ]:
...
class FragmentGraphDataset_point_cloud(torch_geometric.data.Dataset):
  def __init__(
    ...
    dihedral_var=0.0,
    xyz_var=0.0,
    randomize_focal_dihedral=False,
  ):
    ...
    self.next_atom_fragment_indices_sorted_pointer = (
      next_atom_fragment_indices_sorted_pointer
    )

    self.focal_attachment_index_ref_partial_array = (
      focal_attachment_index_ref_partial_array
    )
    self.focal_attachment_point_label_prob_array = (
      focal_attachment_point_label_prob_array
    )
    self.focal_attachment_point_label_prob_pointer = (
      focal_attachment_point_label_prob_pointer
    )
    self.multi_hot_next_atom_fragment_attachment_points_array = (
      multi_hot_next_atom_fragment_attachment_points_array
    )
    self.multi_hot_next_atom_fragment_attachment_points_pointer = (
      multi_hot_next_atom_fragment_attachment_points_pointer
    )
  ...
  def get_number_future_atoms(self, mol, next_atom_index, focal_indices):
    ...
    next_atom_neighbors = tuple([
      a.GetIdx() for a in mol.GetAtomWithIdx(int(next_atom_index)).GetNeighbors()
    ])
    ...
    pieces = rdkit.Chem.FragmentOnSomeBonds(
      mol, bonds, numToBreak=len(bonds), addDummies=False
    )
    disjoint_graphs = rdkit.Chem.GetMolFrags(pieces[0])

    future_atoms = (
      disjoint_graphs[0]
      if (next_atom_index in disjoint_graphs[0])
      else disjoint_graphs[1]
    )

    return float(len(future_atoms))

  def augment_perturb_structure(
    self,
    mol,
    partial_indices,
    focal_indices,
    ...
    rot_bonds_partial = [
      r
      for r in all_rot_bonds
      if ((r[0] in partial_indices) & (r[1] in partial_indices))
    ]
    dihedrals = []
    for bond_ in rot_bonds_partial:
      if int(bond_[0]) == int(focal_indices[0]):
        ...
        p1 = rdkit.Chem.rdmolops.GetShortestPath(
          mol, int(bond_[0]), int(focal_indices[0])
        )
      if int(bond_[1]) == int(focal_indices[0]):
        p2 = ()
      else:
        p2 = rdkit.Chem.rdmolops.GetShortestPath(
          mol, int(bond_[1]), int(focal_indices[0])
        )

      bond = (
        tuple(reversed(bond_)) if len(p1) > len(p2) else bond_
      )  # if bond[0] is farther away, then it should be moved

      first_neighbors = tuple(
        set([a.GetIdx() for a in mol.GetAtomWithIdx(bond[0]).GetNeighbors()])
        - set([bond[1]])
      )
      first_neighbors = [f for f in first_neighbors if f in partial_indices]
      second_neighbors = tuple(
        set([a.GetIdx() for a in mol.GetAtomWithIdx(bond[1]).GetNeighbors()])
        - set([bond[0]])
      )
    ...
    if dihedral_var > 0.0:
      ...
      for d_idx, d in enumerate(dihedrals):
        rdkit.Chem.rdMolTransforms.SetDihedralDeg(
          mol_augmented.GetConformer(),
          *d,
          float(
            angles_1[d_idx]
            + rdkit.Chem.rdMolTransforms.GetDihedralDeg(mol.GetConformer(), *d)
          ),
        )

    if xyz_var > 0.0:
      xyz_augmented = mol_augmented.GetConformer().GetPositions()
      xyz_augmented = xyz_augmented + np.random.normal(
        0, xyz_var, (xyz_augmented.shape[0], 3)
      )
    ...
    if randomize_focal_dihedral == True:
      focal_dihedral = [
        ...
        for d in dihedrals
        if (
          (d[1] in partial_indices)
          & (d[2] in partial_indices)
          & ((d[1] in focal_indices) | (d[2] in focal_indices))
        )
      ]
      if len(focal_dihedral) == 1:
        ...
        rdkit.Chem.rdMolTransforms.SetDihedralDeg(
          mol_augmented.GetConformer(),
          *focal_dihedral_,
          float(np.random.uniform(0, 360, 1)),
        )
  ...
  def __getitem__(self, key):
    ...
    partial_graph_indices = deepcopy(
      self.partial_graph_indices_sorted[
        self.partial_graph_indices_sorted_pointer[
          key
        ] : self.partial_graph_indices_sorted_pointer[key + 1]
      ]
    )
    focal_indices = deepcopy(
      self.focal_indices_sorted[
        self.focal_indices_sorted_pointer[key] : self.focal_indices_sorted_pointer[
          key + 1
        ]
      ]
    )
    next_atom_fragment_indices = deepcopy(
      self.next_atom_fragment_indices_sorted[
        self.next_atom_fragment_indices_sorted_pointer[
          key
        ] : self.next_atom_fragment_indices_sorted_pointer[key + 1]
      ]
    )
    ...
    focal_indices_ref_partial = [
      np.where(partial_graph_indices == f)[0][0] for f in focal_indices
    ]

    focal_attachment_index_ref_partial = deepcopy(
      self.focal_attachment_index_ref_partial_array[key : key + 1]
    )
    focal_attachment_point_label_prob = deepcopy(
      self.focal_attachment_point_label_prob_array[
        self.focal_attachment_point_label_prob_pointer[
          key
        ] : self.focal_attachment_point_label_prob_pointer[key + 1]
      ]
    )
    ...
    edge_index = deepcopy(
      self.edge_index_array[
        :, self.bonds_pointer[original_index] : self.bonds_pointer[original_index + 1]
      ]
    )
    edge_features = deepcopy(
      self.edge_features_array[
        self.bonds_pointer[original_index] : self.bonds_pointer[original_index + 1]
      ]
    )
    node_features = deepcopy(
      self.node_features_array[
        self.atoms_pointer[original_index] : self.atoms_pointer[original_index + 1]
      ]
    )
    atom_fragment_associations = deepcopy(
      self.atom_fragment_associations_array[
        self.atoms_pointer[original_index] : self.atoms_pointer[original_index + 1]
      ]
    )
    ...
    if self.mols != None:
      ...
      if (
        (self.dihedral_var > 0.0)
        | (self.xyz_var > 0.0)
        | (self.randomize_focal_dihedral == True)
      ):
        mol_augmented = self.augment_perturb_structure(
          mol,
          partial_graph_indices,
          focal_indices,
          dihedral_var=self.dihedral_var,
          xyz_var=self.xyz_var,
          randomize_focal_dihedral=self.randomize_focal_dihedral,
        )
      else:
        mol_augmented = mol
    ...
    if self.mols != None:
      positions_augmented = (
        positions_augmented - center_of_mass
      )  # need to subtract the center of mass of the encoded target molecule.

    if next_atom_index != -1:
      next_atom_fragment_ID_index = np.array([
        atom_fragment_associations[next_atom_index]
      ])
      multi_hot_next_atom_fragment_attachment_points = deepcopy(
        self.multi_hot_next_atom_fragment_attachment_points_array[
          self.multi_hot_next_atom_fragment_attachment_points_pointer[
            key
          ] : self.multi_hot_next_atom_fragment_attachment_points_pointer[key + 1]
        ]
      )
      next_atom_fragment_attachment_point_label_prob = (
        multi_hot_next_atom_fragment_attachment_points
        / sum(multi_hot_next_atom_fragment_attachment_points)
      )
      bond_type_class_index_label = deepcopy(
        self.bond_type_class_index_label_array[key : key + 1]
      )
    else:
    ...
    if self.mols != None:
      if next_atom_index != -1:
        num_future_atoms = self.get_number_future_atoms(
          mol, next_atom_index, focal_indices
        )
    ...
    atom_fragment_associations_subgraph = atom_fragment_associations[
      subgraph_node_index
    ]
    ...
    cloud, cloud_batch_indices = get_point_cloud_numpy(
      positions, self.N_points, per_node=True
    )
    cloud_subgraph, cloud_batch_indices_subgraph = get_point_cloud_numpy(
      positions_subgraph, self.N_points, per_node=True
    )
    ...
    data = PairData(
      ...
      atom_fragment_associations=torch.from_numpy(atom_fragment_associations).type(
        torch.long
      ),
      ...
      cloud_index_subgraph=torch.from_numpy(cloud_batch_indices_subgraph).type(
        torch.long
      ),
      cloud_indices_subgraph=torch.from_numpy(cloud_batch_indices_subgraph).type(
        torch.long
      ),
      atom_fragment_associations_subgraph=torch.from_numpy(
        atom_fragment_associations_subgraph
      ).type(torch.long),
      ...
      focal_reindices=torch.from_numpy(np.array(focal_indices_ref_partial)).type(
        torch.long
      ),
      stop_mask=torch.from_numpy(stop_mask).type(torch.bool),
      stop_focal_mask=torch.from_numpy(
        stop_mask.repeat(len(focal_indices_ref_partial))
      ).type(torch.bool),
      focal_attachment_index_subgraph=torch.from_numpy(
        focal_attachment_index_ref_partial
      ).type(torch.long),
      next_atom_type_library_idx=torch.from_numpy(next_atom_fragment_ID_index).type(
        torch.long
      ),
      focal_attachment_point_label_prob=torch.from_numpy(
