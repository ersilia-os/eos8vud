...
beta = (
  float(beta_schedule[beta_iteration])
  if ((variational == True) | (variational_GNN == True) | (variational_GNN_mol == True))
  else None
)
...
if (dihedral_var > 0.0) | (xyz_var > 0.0) | (randomize_focal_dihedral == True):
  if use_artificial_mols:
    filtered_database_mols = list(
      pd.read_pickle(
        "data/MOSES2/MOSES2_training_val_filtered_database_artificial_mols.pkl"
      ).artificial_mols
    )
  else:
    filtered_database_mols = list(
      pd.read_pickle(
        "data/MOSES2/MOSES2_training_val_filtered_database_mols.pkl"
...
AtomFragment_database = pd.read_pickle(
  "data/MOSES2/MOSES2_training_val_AtomFragment_database.pkl"
)
AtomFragment_database = AtomFragment_database.iloc[1:].reset_index(
  drop=True
)  # removing stop token from AtomFragment_database!
...
if shape_penalties:
  ...
  for i in range(len(AtomFragment_database)):
    ...
    if m is not None:
      ...
      volumes[i] = 0.0
  volume_distances = torch.cdist(
    torch.as_tensor(volumes).unsqueeze(1), torch.as_tensor(volumes).unsqueeze(1)
  ).float()
  volume_distances = volume_distances.to(device)

else:
  volume_distances = torch.zeros((
    len(AtomFragment_database),
    len(AtomFragment_database),
  )).float()
...
atom_fragment_associations_array = np.load(
  "data/MOSES2/MOSES2_training_val_atom_fragment_associations_array.npy"
)
...
focal_attachment_index = np.load(
  "data/MOSES2/training_split_arrays/focal_attachment_index.npy"
)
next_atom_index = np.load("data/MOSES2/training_split_arrays/next_atom_index.npy")

partial_graph_indices_sorted = np.load(
  "data/MOSES2/training_split_arrays/partial_graph_indices_sorted.npy"
)
partial_graph_indices_sorted_pointer = np.load(
  "data/MOSES2/training_split_arrays/partial_graph_indices_sorted_pointer.npy"
)

focal_indices_sorted = np.load(
  "data/MOSES2/training_split_arrays/focal_indices_sorted.npy"
)
focal_indices_sorted_pointer = np.load(
  "data/MOSES2/training_split_arrays/focal_indices_sorted_pointer.npy"
)

next_atom_fragment_indices_sorted = np.load(
  "data/MOSES2/training_split_arrays/next_atom_fragment_indices_sorted.npy"
)
next_atom_fragment_indices_sorted_pointer = np.load(
  "data/MOSES2/training_split_arrays/next_atom_fragment_indices_sorted_pointer.npy"
)

focal_attachment_index_ref_partial_array = np.load(
  "data/MOSES2/training_split_arrays/focal_attachment_index_ref_partial_array.npy"
)
focal_attachment_point_label_prob_array = np.load(
  "data/MOSES2/training_split_arrays/focal_attachment_point_label_prob_array.npy"
)
focal_attachment_point_label_prob_pointer = np.load(
  "data/MOSES2/training_split_arrays/focal_attachment_point_label_prob_pointer.npy"
)

multi_hot_next_atom_fragment_attachment_points_array = np.load(
  "data/MOSES2/training_split_arrays/multi_hot_next_atom_fragment_attachment_points_array.npy"
)
multi_hot_next_atom_fragment_attachment_points_pointer = np.load(
  "data/MOSES2/training_split_arrays/multi_hot_next_atom_fragment_attachment_points_pointer.npy"
)

bond_type_class_index_label_array = np.load(
  "data/MOSES2/training_split_arrays/bond_type_class_index_label_array.npy"
)
...
val_focal_attachment_index = np.load(
  "data/MOSES2/validation_split_arrays/focal_attachment_index.npy"
)
val_next_atom_index = np.load("data/MOSES2/validation_split_arrays/next_atom_index.npy")

val_partial_graph_indices_sorted = np.load(
  "data/MOSES2/validation_split_arrays/partial_graph_indices_sorted.npy"
)
val_partial_graph_indices_sorted_pointer = np.load(
  "data/MOSES2/validation_split_arrays/partial_graph_indices_sorted_pointer.npy"
)

val_focal_indices_sorted = np.load(
  "data/MOSES2/validation_split_arrays/focal_indices_sorted.npy"
)
val_focal_indices_sorted_pointer = np.load(
  "data/MOSES2/validation_split_arrays/focal_indices_sorted_pointer.npy"
)

val_next_atom_fragment_indices_sorted = np.load(
  "data/MOSES2/validation_split_arrays/next_atom_fragment_indices_sorted.npy"
)
val_next_atom_fragment_indices_sorted_pointer = np.load(
  "data/MOSES2/validation_split_arrays/next_atom_fragment_indices_sorted_pointer.npy"
)

val_focal_attachment_index_ref_partial_array = np.load(
  "data/MOSES2/validation_split_arrays/focal_attachment_index_ref_partial_array.npy"
)
val_focal_attachment_point_label_prob_array = np.load(
  "data/MOSES2/validation_split_arrays/focal_attachment_point_label_prob_array.npy"
)
val_focal_attachment_point_label_prob_pointer = np.load(
  "data/MOSES2/validation_split_arrays/focal_attachment_point_label_prob_pointer.npy"
)

val_multi_hot_next_atom_fragment_attachment_points_array = np.load(
  "data/MOSES2/validation_split_arrays/multi_hot_next_atom_fragment_attachment_points_array.npy"
)
val_multi_hot_next_atom_fragment_attachment_points_pointer = np.load(
  "data/MOSES2/validation_split_arrays/multi_hot_next_atom_fragment_attachment_points_pointer.npy"
)

val_bond_type_class_index_label_array = np.load(
  "data/MOSES2/validation_split_arrays/bond_type_class_index_label_array.npy"
)
...
if model_state != "":
  ...
  model.load_state_dict(
    torch.load(model_state, map_location=next(model.parameters()).device), strict=True
  )

model.to(device)

logger(
  f"model has {sum([np.prod(p.size()) for p in filter(lambda p: p.requires_grad, model.parameters())])} parameters"
)
...
def loop(
  ...
  shape_penalty_factor=0.0,
  volume_distances=None,
  stop_shape_penalty=0.0,
  variational=False,
  ...
  next_atomFragment_attachment_loss_mask_size = batch_dict[
    "next_atomFragment_attachment_loss_mask_size"
  ]
  ...
  masked_next_atom_attachment_batch_index_reindexed = batch_dict[
  ...
  masked_multihot_next_attachments_label_prob = batch_dict[
    "masked_multihot_next_attachments_label_prob"
  ]
  ...
  if not all_stop:
    ...
    masked_next_atom_attachment_batch_index_reindexed = (
      masked_next_atom_attachment_batch_index_reindexed.to(device)
    )
  ...
  stop_accuracy = torch.mean(
    (torch.round(torch.sigmoid(out[0].squeeze())) == stop_mask.type(torch.float)).type(
      torch.float
    )
  )
  ...
  if not all_stop:
    ...
    stop_shape_loss = torch.mean(
      (1.0 - torch.sigmoid(out[0].squeeze()[stop_mask]))
      * batch.N_future_atoms.squeeze()[stop_mask]
    )
  ...
  if variational:
    ...
    if (variational_mode == "both") | (variational_mode == "equi"):
      KL_unreduced_equi = 0.5 * (
        torch.sum(Z_equi_mean.reshape(batch_size, -1) ** 2.0, dim=1)
        + torch.sum(Z_equi_std.reshape(batch_size, -1) ** 2.0, dim=1)
        - torch.sum(torch.log(Z_equi_std.reshape(batch_size, -1) ** 2.0) + 1.0, dim=1)
      )
    if (variational_mode == "both") | (variational_mode == "inv"):
      KL_unreduced_inv = 0.5 * (
        torch.sum(Z_inv_mean**2.0, dim=1)
        + torch.sum(Z_inv_std**2.0, dim=1)
        - torch.sum(torch.log(Z_inv_std**2.0) + 1.0, dim=1)
      )
    ...
    KL_unreduced = 0.5 * (
      torch.sum(h_mean**2.0, dim=1)
      + torch.sum(h_std**2.0, dim=1)
      - torch.sum(torch.log(h_std**2.0) + 1.0, dim=1)
    )
    ...
    cosine_loss = torch.mean(
      ...
      - torch.nn.functional.cosine_similarity(
        h_reshaped_gnn, h_predicted_reshaped, dim=1
      )
    )
  ...
  if predict_pairwise_properties:
    ...
    pairwise_property_loss = torch.mean(
      torch.square(pairwise_targets - pairwise_properties_out_sigmoid)
    )
  ...
  if not all_stop:
    ...
    next_atomFragment_loss = next_atomFragment_cross_entropy(
      out[1], batch.next_atom_type_library_idx[stop_mask]
    )
    next_atomFragment_accuracy = torch.mean(
      (
        out[1].softmax(dim=1).argmax(dim=1)
        == batch.next_atom_type_library_idx[stop_mask]
      ).type(torch.float)
    )
    ...
    if shape_penalty_factor > 0.0:
      shape_loss = torch.mean(
        out[1].softmax(dim=1)
        * volume_distances[batch.next_atom_type_library_idx[stop_mask]]
      )
    ...
    focal_attachment_losses = torch_scatter.scatter_sum(
      -torch.log(out[2] + 1e-8) * focal_attachment_label_prob_masked,
      masked_focal_batch_index_reindexed,
      dim=0,
    )

    if focal_attachment_loss_mask_size > 0:
      focal_attachment_loss = torch.mean(
        focal_attachment_losses[focal_attachment_loss_mask]
      )  # selecting only those losses from actual fragments (I think)
    ...
    if next_atomFragment_attachment_loss_mask_size > 0:
      binary_next_attachment_point_losses = torch_scatter.scatter_add(
        out[3][select_multi_losses], mask_select_multi
      )
      next_atomFragment_attachment_loss = -torch.mean(
        torch.log(binary_next_attachment_point_losses + 1e-8)
      )
  ...
  return (
    batch_size,
    stop_loss.item(),
    next_atomFragment_loss.item(),
    focal_attachment_loss.item(),
    next_atomFragment_attachment_loss.item(),
    bond_loss.item(),
    sum(stop_mask.cpu().numpy()),
    focal_attachment_loss_mask_size,
    next_atomFragment_attachment_loss_mask_size,
    sum(stop_mask.cpu().numpy()),
    stop_accuracy.item(),
    next_atomFragment_accuracy.item(),
    batch.x.shape[0] // batch_size,
    batch.x_subgraph.shape[0] // batch_size,
    shape_loss.item(),
    KL_loss.item(),
    stop_shape_loss.item(),
    cosine_loss.item(),
    pairwise_property_loss.item(),
    N_pairwise_targets,
    mol_prop_loss.item(),
  )
...
scheduler = (
  torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma) if use_scheduler else None
)
...
for epoch in range(1, 1 + N_epochs):
  if validate_only == False:
    ...
    for b, batch in enumerate(train_loader):
      (
        ...
        training=training,
        device=device,
        shape_penalty_factor=shape_penalty_factor,
        volume_distances=volume_distances,
        stop_shape_penalty=stop_shape_penalty,
        variational=variational,
        variational_mode=variational_mode,
        variational_GNN=variational_GNN,
        variational_GNN_mol=variational_GNN_mol,
        ...
        beta=beta,
        predict_pairwise_properties=predict_pairwise_properties,
        pairwise_property_factor=pairwise_property_factor,
        predict_mol_property=predict_mol_property,
        mol_property_factor=mol_property_factor,
      )

      if iteration == 0:
        logger(
          f"stopToken_loss: {stopToken_loss}, next_loss: {next_loss}, focal_loss: {focal_loss}, attachment_loss: {attachment_loss}, bond_loss: {bond_loss}, shape_loss: {shape_loss}, KL_loss: {KL_loss}, stop_shape_loss: {stop_shape_loss}, pairwise_property_loss: {pairwise_property_loss}, mol_prop_loss: {mol_prop_loss}"
        )
      ...
      if (iteration % interval) == 0:
        train_stop_loss.append(
          float(np.nansum(np.array(stop_losses) * np.array(batch_sizes)))
          / sum(np.array(batch_sizes))
        )
        train_next_loss.append(
          float(np.nansum(np.array(next_losses) * np.array(N_next_losses)))
          / sum(np.array(N_next_losses))
        )
        train_focal_loss.append(
          float(np.nansum(np.array(focal_losses) * np.array(N_focal_losses)))
          / sum(np.array(N_focal_losses))
        )
        train_attachment_loss.append(
          float(np.nansum(np.array(attachment_losses) * np.array(N_attachment_losses)))
          / sum(np.array(N_attachment_losses))
        )
        train_bond_loss.append(
          float(np.nansum(np.array(bond_losses) * np.array(N_bond_losses)))
          / sum(np.array(N_bond_losses))
        )
        train_epoch_number.append(epoch)
        train_shape_loss.append(
          float(np.nansum(np.array(shape_losses) * np.array(N_next_losses)))
          / sum(np.array(N_next_losses))
        )
        train_KL_loss.append(
          float(np.nansum(np.array(KL_losses) * np.array(batch_sizes)))
          / sum(np.array(batch_sizes))
        )
        train_stop_shape_loss.append(
          float(np.nansum(np.array(stop_shape_losses) * np.array(N_next_losses)))
          / sum(np.array(N_next_losses))
        )
        train_cosine_loss.append(
          float(np.nansum(np.array(cosine_losses) * np.array(batch_sizes)))
          / sum(np.array(batch_sizes))
        )
        train_pairwise_property_loss.append(
          float(
            np.nansum(
              np.array(pairwise_property_losses) * np.array(N_pairwise_targets_sizes)
            )
          )
          / sum(np.array(N_pairwise_targets_sizes))
        )
        train_mol_prop_loss.append(
          float(np.nansum(np.array(mol_prop_losses) * np.array(batch_sizes)))
          / sum(np.array(batch_sizes))
        )
      ...
      if (save) & ((iteration % save_interval) == 0):
        ...
        torch.save(
          model.state_dict(),
          PATH + f"saved_models/model_{int(iteration / save_interval)}.pt",
        )
      ...
      if (
        ...
        | (variational_GNN_mol == True)
      ) & (iteration % beta_interval == 0):
        beta_iteration += 1
  ...
  val_stop_loss.append(
    float(np.nansum(np.array(val_stop_losses) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )
  val_next_loss.append(
    float(np.nansum(np.array(val_next_losses) * np.array(val_N_next_losses)))
    / sum(np.array(val_N_next_losses))
  )
  val_focal_loss.append(
    float(np.nansum(np.array(val_focal_losses) * np.array(val_N_focal_losses)))
    / sum(np.array(val_N_focal_losses))
  )
  val_attachment_loss.append(
    float(
      np.nansum(np.array(val_attachment_losses) * np.array(val_N_attachment_losses))
    )
    / sum(np.array(val_N_attachment_losses))
  )
  val_bond_loss.append(
    float(np.nansum(np.array(val_bond_losses) * np.array(val_N_bond_losses)))
    / sum(np.array(val_N_bond_losses))
  )
  val_epoch_number.append(epoch)

  val_stop_accuracy.append(
    float(np.nansum(np.array(val_stop_accuracies) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )
  val_next_accuracy.append(
    float(np.nansum(np.array(val_next_accuracies) * np.array(val_N_next_losses)))
    / sum(np.array(val_N_next_losses))
  )
  # val_mol_size.append()
  # val_subgraph_size.append()
  val_shape_loss.append(
    float(np.nansum(np.array(val_shape_losses) * np.array(val_N_next_losses)))
    / sum(np.array(val_N_next_losses))
  )
  val_KL_loss.append(
    float(np.nansum(np.array(val_KL_losses) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )
  val_stop_shape_loss.append(
    float(np.nansum(np.array(val_stop_shape_losses) * np.array(val_N_next_losses)))
    / sum(np.array(val_N_next_losses))
  )
  val_cosine_loss.append(
    float(np.nansum(np.array(val_cosine_losses) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )
  val_pairwise_property_loss.append(
    float(
      np.nansum(
        np.array(val_pairwise_property_losses) * np.array(val_N_pairwise_targets_sizes)
      )
    )
    / sum(np.array(val_N_pairwise_targets_sizes))
  )
  val_mol_prop_loss.append(
    float(np.nansum(np.array(val_mol_prop_losses) * np.array(val_batch_sizes)))
    / sum(np.array(val_batch_sizes))
  )

  val_logger(
    f"iteration: {iteration}, stop_loss:{val_stop_loss[-1]}, next_loss: {val_next_loss[-1]}, focal_loss: {val_focal_loss[-1]}, attachment_loss: {val_attachment_loss[-1]}, bond_loss: {val_bond_loss[-1]}, stop_accuracy: {val_stop_accuracy[-1]}, next_accuracy: {val_next_accuracy[-1]}, shape_loss: {val_shape_loss[-1]}, KL_loss: {val_KL_loss[-1]}, stop_shape_loss: {val_stop_shape_loss[-1]}, cosine_loss: {val_cosine_loss[-1]}, pairwise_property_loss: {val_pairwise_property_loss[-1]}, mol_prop_loss: {val_mol_prop_loss[-1]}"
  )
